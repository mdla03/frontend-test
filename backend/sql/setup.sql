...
-- ---------------------------------------------------------------------------
-- Demo accounts. Password for all six: demo1234 -- CHANGE before anything real.
-- crypt()/gen_salt('bf') produces a $2a$ bcrypt hash that bcryptjs verifies.
-- Re-running this block resets the passwords, so it also repairs a bad seed.
-- ---------------------------------------------------------------------------
insert into users (name, email, role, organizer_status, password)
values
  ('Mark Aquino',    'mark.aquino@manulife.com',  'user',      'none',     crypt('demo1234', gen_salt('bf', 10))),
  ('Liza Reyes',     'liza.reyes@manulife.com',   'organizer', 'approved', crypt('demo1234', gen_salt('bf', 10))),
  ('Carlo Santos',   'carlo.santos@manulife.com', 'admin',     'approved', crypt('demo1234', gen_salt('bf', 10))),
...
-- Sanity check: 6 rows, all bcrypt.
select email, role, left(password, 4) as hash_prefix from users order by email;
